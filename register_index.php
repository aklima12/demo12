            ...
            if(strlen($username)>=5){
                if(strlen($password)>=8 && strlen($c_password)>=8){
                        if($password==$c_password){
                                $user_check=mysqli_query($admin_dbcon,"SELECT * FROM `from` WHERE `username`='$username'");
                                if(mysqli_num_rows($user_check)==0){
                                    $email_check=mysqli_query($admin_dbcon,"SELECT * FROM `from` WHERE `email`='$email'");     
                                        if(mysqli_num_rows($email_check)==0){
                                                
                                                date_default_timezone_set("Asia/Dhaka");
                                                $reg_time=date('m-d-Y,h:i:s a');
                                            $insert_query=mysqli_query($admin_dbcon,"INSERT INTO `from`(`username`, `email`, `mobile`, `password`, `c_password`, `reg_time`, `status`) VALUES ('$username','$email','$mobile','$password','$c_password','$reg_time','Inactive')");
                                                if($insert_query){
                                                    echo '<script>
